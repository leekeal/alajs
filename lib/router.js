...
Router = function(){
	this.autoRoute = function(){
		...
		// app.get('/:ctrl?/:sub?/:action?',function(req, res,next){
		...
		// app.put('/:ctrl?/:sub?/:action?',function(req, res,next){
		// 	var ctrl = req.params.ctrl;
		// 	var sub = req.params.sub;
		// 	var action = req.params.action
		// 	 /*ctrl存在   ctrl存在是对象   sub控制器存在是对象   sub的action是函数*/
		// 	 if (ctrl && typeof ala.controllers[ctrl] == 'object' && typeof ala.controllers[ctrl][sub||'index'] == 'object' && typeof ala.controllers[ctrl][sub||'index']["put"+(action||'Index')] == 'function'){
		// 	 	sub = sub||'index';
		// 	 	action = "put"+(action||'Index');
		// 	 	app.put(req.path,ala.controllers[ctrl][sub][action])
		// 	 	ala.echoInfo('put->  '+req.path+" ===> "+ctrl+"."+(sub)+"."+action)
		// 	 }
		// 	 /*ctrl存在  ctrl控制器存在是对象 sub作为action存在是函数*/
		// 	 else if(ctrl && typeof ala.controllers[ctrl] == 'object' && typeof ala.controllers[ctrl]["put"+(sub||'Index')] == 'function'){
		// 	 	action = "put"+(sub||'Index');
		// 	 	app.put(req.path,ala.controllers[ctrl][action])
